...
/*
 * picdisable() - This routine
 *		* Clears table of interrupt vectors
 *		* Sets all interrupt priority levels to 0 (ignored)
 *		* Masks all interrupts out with 0
 *		* Sets interrupt vectors of PICs to 0x20-0x2f
 *		* Configures all interrupts to be disabled
 *
 *	At the end of this fcn the interrupt functionality of the system should be
 *	ready for IO APIC to take control instead of legacy PIC.
 *
 */
...
picdisable(void)
{
	...
	unsigned int i;
	for (i = SPL0; i < NSPL; i++) {
		pic_mask[i] = PIC_MASK_ZERO;
	}

	/*
	** 1. Use priority level 0 (ignored)
	*/

	curr_ipl = SPL0;
	curr_pic_mask = pic_mask[SPL0];
